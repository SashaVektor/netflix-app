...
export const fetchMovies = createAsyncThunk('netflix/fetchMovies', async ({ type }, thunkApi) => {
    const { netflixSlice: { genres } } = thunkApi.getState();
...
export const getUserLikedMovies = createAsyncThunk('netflix/getUserLikedMovies', async (email) => {
    const {data: {movies}} = await axios.get(`http://localhost:5000/api/user/liked/${email}`)
    return movies
})

export const removeFromLikedMovies = createAsyncThunk('netflix/removeFromLikedMovies', async ({email, movieId}) => {
